                    ...
                    .map((product, productIndex) => (
                      <div key={productIndex} className="product-slide-card">
                        <div className="product-slide-image">
                          <img 
                            src={product.image || `https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=400&h=400&fit=crop&q=80&auto=format`} 
                            alt={product.name}
                            loading="lazy"
                          />
                        </div>
                        <div className="product-slide-info">
                          <h3 className="product-slide-name">{product.name}</h3>
                          <div className="product-slide-rating">
                            {[...Array(5)].map((_, i) => (
                              <span key={i} className={`star ${i < (product.rating || 0) ? 'filled' : 'outline'}`}>
                                {i < (product.rating || 0) ? '★' : '☆'}
                              </span>
                            ))}
                          </div>
